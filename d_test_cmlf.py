...
def main():
    model_path = os.path.join('results', 'wo-cm')
    args = load_args_from_file(os.path.join(model_path, 'args.txt'))
    cmlf = CMLF(args)
    dataset_type = 'test' # val

    # Data Loader
    print('Loading Dataset')
    test_dir = os.path.join('dataset', 'cm_dataset', dataset_type+'_set')
    ...
    cmlf.load_state_dict(state_dict, strict=False)
    ...
    np.savez(os.path.join(model_path, 'outf_' + dataset_type + '_n' + str(noise) + '_c' + str(corruption) + '.npz'),
