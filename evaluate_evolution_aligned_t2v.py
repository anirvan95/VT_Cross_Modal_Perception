...
for model_out in model_outs:
    ...
    tac_y_kernel_pls = safe_fit(tac_y_kernel_pls, tac_y_params[:, -1, :, 0], labels[:, 0:3])
    tac_z_kernel_pls = safe_fit(tac_z_kernel_pls, tac_z_params[:, -1, :, 0], labels[:, 0:3])
    ...
    for obj in range(len(obj_inds) - 1):
        ...
        for action in action_inds:
            ...
            if generate_plots:
                ...
                # Improve layout
                plt.savefig(os.path.join('results', 'crossmodal', cm_type, 'evolution_aligned', 'tactile',
        ...
        for action in action_inds:
            ...
            if generate_plots:
                ...
                for i in range(3):
                    ...
                    # Improve layout
                plt.savefig(os.path.join('results', 'crossmodal', cm_type, 'evolution_aligned', 'vision', 'Obj-' + str(obj) + '-a-' + str(action) + '.png'))
                plt.close()
                ...
                # Improve layout
                plt.savefig(os.path.join('results', 'crossmodal', cm_type, 'evolution_aligned', 'vision', 'ObjPose-' + str(obj) + '-a-' + str(action) + '.png'))
                plt.close()

    global_error_y_vis = np.array(global_error_y_vis)
    global_error_z_vis = np.array(global_error_z_vis)
    np.save(os.path.join('results', 'crossmodal', cm_type, 'final_errors', 'global_error_vis_y'+condition_type+'.npy'), global_error_y_vis)
    np.save(os.path.join('results', 'crossmodal', cm_type, 'final_errors', 'global_error_vis_z'+condition_type+'.npy'), global_error_z_vis)

    global_error_y_tac = np.array(global_error_y_tac)
    global_error_z_tac = np.array(global_error_z_tac)
    np.save(os.path.join('results', 'crossmodal', cm_type, 'final_errors', 'global_error_tac_y'+condition_type+'.npy'), global_error_y_tac)
    np.save(os.path.join('results', 'crossmodal', cm_type, 'final_errors', 'global_error_tac_z'+condition_type+'.npy'), global_error_z_tac)
