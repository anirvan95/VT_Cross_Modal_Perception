...
def main():
    cmlf_base_results_path = os.path.join('results', 'crossmodal', 't2v_late')
    args = load_args_from_file(os.path.join(cmlf_base_results_path, 'args.txt'))
    noises = [0.125] # std deviation of the noise to be passed
            ...
            with torch.no_grad():
                for values in test_loader:
                    ...
                    object_labels = labels[:, :,
                                    6:]  # Required for hierarchical prior, contrastive version is more general
                    pose_labels = labels[:, :, :6]  # GT Pose of the object
