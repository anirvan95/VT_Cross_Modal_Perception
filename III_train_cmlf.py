...
def compute_elbo(cmlf, vis_obs, tac_obs, actions, labels, hyperparam, H):
    ...
    object_labels = labels[:, :, 6:]    # Required for hierarchical prior
    ...
    while iteration < num_iterations:
        for i, values in enumerate(train_loader):
            ...
            if iteration % args.log_freq == 0:
                ...
                if args.validate:
                    cmlf.eval()
                    validate_cmlf(val_loader, cmlf, out_dir, H, iteration, save_plot=args.saveplots, show_plot=args.showplots, vis=vis)
