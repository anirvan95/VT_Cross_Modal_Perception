...
class PCMC(nn.Module):
    def __init__(self, dim_x: Tuple, dim_u: int, dim_z: int, dim_alpha: int, dim_beta: int, batch_size: int, hidden_transition: int = 128, hidden_size=128, device='cuda'):
        ...
        self.vae = VAE(beta_dim=15, gamma_dim=15, use_cuda=use_cuda, prior_dist=prior_dist, q_dist=q_dist)
    ...
    def get_initial_samples(self, x):
        ...
        alphas = torch.zeros((x.shape[0], self.dim_alpha)).to(self.device)
    ...
    def sample_beta(self, x_t=None):
        if x_t is not None:
            ...
            mu_beta = torch.zeros((self.batch_size, self.dim_beta)).to(self.device)
            logvar_beta = torch.ones((self.batch_size, self.dim_beta)).to(self.device)
