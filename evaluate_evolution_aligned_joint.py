...
def safe_fit(model, X, Y):
    # Flatten spatial dims if needed (optional, depending on your shape)
    X_clean = X
    Y_clean = Y

    # Find rows that have no NaNs in X or Y
    valid_rows = ~np.isnan(X_clean).any(axis=1) & ~np.isnan(Y_clean).any(axis=1)

    # Filter valid rows
    X_clean = X_clean[valid_rows]
    Y_clean = Y_clean[valid_rows]
    print('Diff: ', len(X_clean)-len(X))
    if len(X_clean) > 0:
...
action_inds = list(range(0, 48, 1)) # Do it for 12 interaction parameter
cm_type = 'vt'
save_path_type = 'surprise_error'
cmlf_output_path = os.path.join('results', 'joint', cm_type, 'surprise_output')
...
for model_out in model_outs:
    ...
    unique_object_labels = ((labels_test[:, :, 0] - 1) * 15 + (labels_test[:, :, 1] - 1) * 5 + (labels_test[:, :, 2] - 1))
    ...
    obj_inds = list(range(0, y_params.shape[0]+48, 48))
    ...
    vis_y_kernel_pls = safe_fit(vis_y_kernel_pls, y_params[:, -1, :, 0], labels[:, 0:3])
    tac_y_kernel_pls = safe_fit(tac_y_kernel_pls, y_params[:, -1, :, 0], labels[:, 3:])
    tac_z_kernel_pls = safe_fit(tac_z_kernel_pls, tac_z_params[:, -1, :, 0], labels[:, 3:])
    ...
    for obj in range(len(obj_inds) - 1):
        print(obj)
        sel_y_params = y_params[obj_inds[obj]:obj_inds[obj + 1]]
        ...
        for action in action_inds:
            ...
            for t in range(num_time_steps-1):
                ...
                mu_y = y_params[action, t, :, 0]
                sigma_y = np.exp(y_params[action, t, :, 1])

                samples_y = np.random.normal(mu_y, sigma_y, size=(num_samples, sel_y_params.shape[2]))
            ...
            if generate_plots:
                ...
                for i in range(3):
                    ...
                    ax[i].plot(pred_mean, colors[i][0], linestyle='-.', label="Predicted Mean")
                ...
                # Improve layout
                plt.savefig(os.path.join('results', 'joint', cm_type, 'evolution_aligned', 'tactile',
                                         'ObjZ-' + str(obj) + '-a-' + str(action) + '.png'))
                plt.close()
        ...
        for action in action_inds:
            ...
            for t in range(num_time_steps - 1):
                ...
                samples_y = np.random.normal(mu_y, sigma_y, size=(num_samples, y_params.shape[2]))
            ...
            if generate_plots:
                ...
                for i in range(3):
                    ...
                    # Improve layout
                plt.savefig(os.path.join('results', 'joint', cm_type, 'evolution_aligned', 'vision', 'Obj-' + str(obj) + '-a-' + str(action) + '.png'))
                plt.close()
                ...
                # Improve layout
                plt.savefig(os.path.join('results', 'joint', cm_type, 'evolution_aligned', 'vision', 'ObjPose-' + str(obj) + '-a-' + str(action) + '.png'))
                plt.close()

    global_error_y_vis = np.array(global_error_y_vis)
    global_error_z_vis = np.array(global_error_z_vis)
    np.save(os.path.join('results', 'joint', cm_type, save_path_type, 'global_error_vis_y'+condition_type+'.npy'), global_error_y_vis)
    np.save(os.path.join('results', 'joint', cm_type, save_path_type, 'global_error_vis_z'+condition_type+'.npy'), global_error_z_vis)

    global_error_y_tac = np.array(global_error_y_tac)
    global_error_z_tac = np.array(global_error_z_tac)
    np.save(os.path.join('results', 'joint', cm_type, save_path_type, 'global_error_tac_y'+condition_type+'.npy'), global_error_y_tac)
    np.save(os.path.join('results', 'joint', cm_type, save_path_type, 'global_error_tac_z'+condition_type+'.npy'), global_error_z_tac)
