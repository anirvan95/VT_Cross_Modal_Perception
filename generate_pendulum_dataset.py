...
def collect_data(num_sequences: int, sequence_length: int):
    ...
    '''
    l = np.linspace(0.5, 1.5, 1)    # modify the variation in length of pendulum
    m = np.linspace(0.1, 1.0, 1)    # modify the variation in mass of pendulum
    mu = np.linspace(0.1, 0.5, 1)  # modify the variation in joint friction of pendulum
    ...
    np.savez(f'datasets/pendulum/training.npz', obs=np.array(data['obs']), actions=np.asarray(data['actions']), states=np.array(data['states']), parameters=np.array(data['parameters']))


if __name__ == '__main__':
    collect_data(15000, 50)