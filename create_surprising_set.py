...
def parse_file(file_name):
    ...
    for interaction in range(0, len(object_db), action_skip):
        ...
        match = re.match(r'([a-z])-([0-9]+)-([0-9]+)\.pickle', file_name)
    ...
    return vis_obs_np, tac_obs_np, action_np, gt_obs_np
...
save_path = 'dataset/cm_dataset/testing'
action_skip = 1 # check this
...
# Illusion on Shape - Stiffness
# Illusion on Color - Friction
# Illusion on Size - Weight

visual_objects = ['c-1-1.pickle', 'c-2-2.pickle', 'c-3-3.pickle', 'c-1-3.pickle',
                  'h-1-1.pickle', 'h-2-4.pickle', 'h-3-5.pickle', 'h-1-3.pickle',
                  's-2-5.pickle', 'r-2-5.pickle','e-2-5.pickle',
                  's-2-1.pickle', 'r-2-1.pickle','e-2-1.pickle',
                  'e-1-2.pickle', 'c-1-2.pickle', 'r-1-3.pickle', 's-1-4.pickle',
                  'e-3-2.pickle', 'c-3-2.pickle', 'r-3-3.pickle', 's-3-4.pickle']
tactile_objects = ['h-1-1.pickle', 'h-2-2.pickle', 'h-3-3.pickle', 'h-1-3.pickle',
                   'c-1-1.pickle', 'c-2-4.pickle', 'c-3-5.pickle', 'c-1-3.pickle',
                   's-2-1.pickle', 'r-2-1.pickle','e-2-1.pickle',
                   's-2-5.pickle', 'r-2-5.pickle','e-2-5.pickle',
                   'e-3-2.pickle', 'c-3-2.pickle', 'r-3-3.pickle', 's-3-4.pickle',
                   'e-1-2.pickle', 'c-1-2.pickle', 'r-1-3.pickle', 's-1-4.pickle']

for i in range(0, len(visual_objects)):
    print('Creating obj - ', i)
    vis_obs_np, _, _, vis_gt_obs_np = parse_file(visual_objects[i])
    _, tac_obs_np, action_np, tac_gt_obs_np = parse_file(tactile_objects[i])

    vis_obs_np = np.array(vis_obs_np)
    tac_obs_np = np.array(tac_obs_np)
    action_torch = np.array(action_np)
    vis_gt_obs_torch = np.array(vis_gt_obs_np)
    tac_gt_obs_torch = np.array(tac_gt_obs_np)

    gt_obs_torch = np.concatenate((vis_gt_obs_torch[:, :, 0:9], tac_gt_obs_torch[:, :, 9:]), axis=-1)

    vis_obs_torch = normalize(vis_obs_np, min_viz, max_viz)
    tac_obs_torch = normalize(tac_obs_np, min_tac, max_tac)

    print('Saving data')
    test_file_path = os.path.join(save_path, 'training_'+str(i)+'.npz')
