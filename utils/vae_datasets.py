...
class Pendulum(object):
    def __init__(self, dataset_zip=None):
        ...
        # Merge time horizon to samples
        self.imgs = self.imgs.view(num_trajectories*15, 32, 32)
        self.states = self.states.view(num_trajectories*15, 2)  # theta, theta_dot
        self.params = self.params.view(num_trajectories*15, 3)  # length, mass, joint_friction

    def __len__(self):
        return self.imgs.size(0)

    def __getitem__(self, index):
        obs = self.imgs[index].view(1, 32, 32)
        state = self.states[index].view(1, 2)
        parameter = self.params[index].view(1, 3)
        return obs, state, parameter
