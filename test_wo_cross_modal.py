...
def main():
    cmlf_base_results_path = os.path.join('results', 'wocrossmodal', 'vt')
    args = load_args_from_file(os.path.join(cmlf_base_results_path, 'args.txt'))
    noises = [0] # std deviation of the noise to be passed
    corruptions = [] # ratio of time horizon data is corrupted
    ...
    test_loader = DataLoader(dataset=val_dataset, batch_size=48, shuffle=False)
    ...
    checkpoints = [checkpoints[-1]] # last checkpoint
    for i, checkpoint in enumerate(checkpoints):
        ...
        H = 0
        for noise in noises:
            print('Processing noise {}'.format(noise))
            corruption = 0
            first = True
            with torch.no_grad():
                for values in test_loader:
                    ...
                    (vis_prior_params_y_f, vis_y_params_f, vis_ys_f, vis_prior_params_z_f, vis_z_params_f, vis_zs_f,
                     ...
                     tac_xs_hat_f,
                     tac_x_hat_params_f, tac_x_f, tac_hs_f, tac_cs_f, vis2tac_y_params_f, lsttac_y_params_f) = cmlf.filter(vis_obs, tac_obs, actions,
                                                                                    object_labels, H)
            ...
            np.savez(os.path.join(cmlf_base_results_path, 'final_output', str(iteration) + '_n'+str(noise)+'_c'+str(corruption)+'.npz'),
        ...
        for corruption in corruptions:
            print('Processing corruption {}'.format(corruption))
            noise = 0
            first = True
            with torch.no_grad():
                for values in test_loader:
                    ...
                    (vis_prior_params_y_f, vis_y_params_f, vis_ys_f, vis_prior_params_z_f, vis_z_params_f, vis_zs_f,
                     ...
                     tac_prior_params_y_f, tac_y_params_f, tac_ys_f, tac_prior_params_z_f, tac_z_params_f, tac_zs_f,
                     tac_xs_hat_f,
                     tac_x_hat_params_f, tac_x_f, tac_hs_f, tac_cs_f, tac2vis_y_params_f,
                     lstvis_y_params_f) = cmlf.filter(vis_obs, tac_obs, actions,
                                                      object_labels, H)
            ...
            np.savez(os.path.join(cmlf_base_results_path, 'final_output',
                                  str(iteration) + '_n' + str(noise) + '_c' + str(corruption) + '.npz'),
                     vis_y_params=vis_y_params,
                     vis_z_params=vis_z_params,
                     tac_y_params=tac_y_params,
                     tac_z_params=tac_z_params,
