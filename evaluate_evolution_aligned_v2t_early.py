...
for model_out in model_outs:
    ...
    tac_y_kernel_pls.fit(tac_y_params[:, -1, :, 0], labels[:, 0:3])
    tac_z_kernel_pls.fit(tac_z_params[:, -1, :, 0], labels[:, 0:3])
    ...
    for obj in range(len(obj_inds) - 1):
        ...
        for action in action_inds:
            ...
            if generate_plots:
                ...
                # Improve layout
                plt.savefig(os.path.join('results', 'crossmodal', 'v2t_late', 'evolution_aligned', 'vision', 'ObjPose-' + str(obj) + '-a-' + str(action) + '.png'))
                plt.close()
