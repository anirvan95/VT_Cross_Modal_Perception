...
class VAE(nn.Module):
    ...
    def model_sample(self, batch_size=1):
        ...
        # TODO: check from only prior params gamma
        # prior_params_gamma = self._get_prior_params_gamma(batch_size)
    ...
    def decode(self, gamma):
        x_params = self.decoder.forward(gamma).view(gamma.size(0), 1, 64, 64)
    ...
    def reconstruct_img(self, x):
        ...
        gamma_params = self.gamma_func(betas).view(betas.size(0), self.gamma_dim, self.q_dist_gamma.nparams)
        gammas = self.q_dist_gamma.sample(params=gamma_params)
        xs, x_params = self.decode(gammas)
        return xs, x_params, betas, beta_params, gammas, gamma_params
...
def main():
    ...
    vae = VAE(beta_dim=6, gamma_dim=6, use_cuda=False, prior_dist=prior_dist, q_dist=q_dist, include_mutinfo=not args.exclude_mutinfo, tcvae=args.tcvae, conv=False, mss=args.mss).to(device)
