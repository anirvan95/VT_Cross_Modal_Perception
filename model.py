...
class DVBF(nn.Module):
    def __init__(self, dim_x: Tuple, dim_u: int, dim_z: int, dim_w: int, num_matrices: int = 16, hidden_size=128):
        ...
        self.v_params = nn.Sequential(
            nn.Linear(in_features=dim_z+dim_u, out_features=num_matrices),
            #nn.ReLU(),
        ...
        # self.A = nn.Parameter(torch.randn([num_matrices, dim_z, dim_z])*(1.0/self.dim_z))
        # self.B = nn.Parameter(torch.randn([num_matrices, dim_z, dim_u])*(1.0/np.sqrt(self.dim_z*self.dim_u)))
        # self.C = nn.Parameter(torch.randn([num_matrices, dim_z, dim_w])*(1.0/np.sqrt(self.dim_z*self.dim_w)))
    ...
    def sample_w(self, z_t, u_t, x_t=None):
        if x_t is not None:
            ...
            mu_w, logstd_w = torch.split(w_params, split_size_or_sections=self.dim_w, dim=1)
        else:
            mu_w = torch.zeros((z_t.shape[0], self.dim_w)).to(z_t)
            logstd_w = torch.ones((z_t.shape[0], self.dim_w)).to(z_t)
        return mu_w, logstd_w
    ...
    def filter(self, x: torch.Tensor, u: torch.Tensor):
        ...
        for t in range(1, T):
            u_t = u[:, t - 1]
            if t < num_obs:
                z_t, mu_w_t, logstd_w_t, w_t = self.forward(z=z_t, u=u_t, x=x[:, t], return_q=True)
            else:
                z_t, mu_w_t, logstd_w_t, w_t = self.forward(z=z_t, u=u_t, return_q=True)
            z.append(z_t)
            w.append(w_t)
            w_means.append(mu_w_t)
            w_stds.append(logstd_w_t)
    ...
    def forward(self, z: torch.Tensor, u: torch.Tensor, x: torch.Tensor = None, return_q=False):
        ...
        mu_w, logstd_w = self.sample_w(z, u, x)
        w = self.generate_samples(mu_w, logstd_w)
        z = (A @ z.unsqueeze(-1) + B @ u.unsqueeze(-1) + C @ w.unsqueeze(-1)).squeeze(-1)
        if return_q:
            return z, mu_w, logstd_w, w
        else:
            return z
    ...
    def loss(self, x, u):
        ...
        w_means, w_logstds, w = info['w_means'], info['w_stds'], info['betas']
        ...
        # rec_loss = torch.mean((x_hat - x.view(-1, self.dim_x))**2)
        rec_loss = torch.nn.functional.mse_loss(x_hat, x.view(-1, self.dim_x))
        # rec_loss = torch.nn.functional.cross_entropy(x_hat, x.view(-1, self.dim_x))
        #rec_loss = -logprob_x.mean()
        w_mean, w_logstds = w_means.view(-1, self.dim_w), w_logstds.view(-1, self.dim_w)
