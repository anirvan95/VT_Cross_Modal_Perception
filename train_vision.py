...
def compute_elbo(umlf, obs, action, labels, hyperparam, H):
    """
    Computes the ELBO for visual modality, utilises GT pose for efficient latent space learning
    :param umlf: visual latent filter model
    :param obs: visual observation
    ...
    vis_object_labels = labels[:, :, 6:9] # Required for prior TODO: fix this, unifying into 1 single label
    pose_labels = labels[:, :, :6] # GT Pose of the object

    prior_params_y_f, y_params_f, ys_f, prior_params_z_f, z_params_f, zs_f, xs_hat_f, x_hat_params_f, x_f, hs_f, cs_f = umlf.filter(obs, action, vis_object_labels, H)
    ...
    logqz_pose = umlf.vae.q_dist.log_density(pose_labels, params=z_params[:, :, :6, :]).view(batch_size, -1).sum(1)
    ...
    elbo = torch.mean(logpx)*0.1 +  torch.mean(logqz_pose)*100 + hyperparam['lamb']*(torch.mean(logpz - logqz_condx)) + hyperparam['beta']*torch.mean(logpy - logqy_condx)
...
def main():
    ...
    parser.add_argument('--modality', default='vision', type=str, choices=['vision', 'tactile'])
    ...
    parser.add_argument('--save', default='results/unimodallf/vision', help='Path to save the models')
    parser.add_argument('--debug_dir', default='dump/training/unimodallf/vision', help='Path to save the dump files')
    ...
    # setup the optimizer
    optimizer = optim.Adam(vision_lf.parameters(), lr=args.learning_rate)
    ...
    while iteration < num_iterations:
        for i, values in enumerate(train_loader):
            vis_obs, _, actions, labels = values
            ...
            reconstructed, obj, elbo = compute_elbo(vision_lf, vis_obs, actions, labels, hyperparam, 1)
            ...
            if iteration % args.log_freq == 0:
                ...
                utils.save_checkpoint({
                    'state_dict': vision_lf.state_dict(),
                    'args': args}, out_dir, iteration)

                if args.visdom:
                    # For real time evaluation, slightly faster
                    plot_lf(vision_lf, vis, x_test, a_test, label_test, label_scatter, color_vals)
                elif args.plotting:
                    # For tuning of parameters
                    save_lf(vision_lf, x_test, a_test, label_test, label_scatter, color_vals, iteration, out_dir)
