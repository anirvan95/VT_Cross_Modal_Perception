...
from utils.plot_latent import validate_baseline
...
def compute_elbo(svae, vis_obs, tac_obs, actions, labels, hyperparam):
    """
    ...
    (prior_params_y_f, y_params_f, ys_f, hs_f, cs_f,
     ...
     tac_prior_params_z_f, tac_z_params_f, tac_zs_f, tac_xs_hat_f, tac_x_hat_params_f, tac_x_f) = svae.filter(vis_obs, tac_obs, actions, object_labels)

    prior_params_y = torch.stack(prior_params_y_f, dim=1).view(batch_size, (T - 1), svae.dim_y, 2)
    y_params = torch.stack(y_params_f, dim=1).view(batch_size, (T - 1), svae.dim_y, 2)
    ys = torch.stack(ys_f, dim=1).view(batch_size, (T - 1), svae.dim_y)
    logpy = svae.prior_dist_y.log_density(ys, params=prior_params_y).view(batch_size, -1).sum(1)
    logqy_condx = svae.q_dist_y.log_density(ys, params=y_params).view(batch_size, -1).sum(1)

    # ############################################### Vision #######################################
    vis_prior_params_z = torch.stack(vis_prior_params_z_f, dim=1).view(batch_size, T, svae.vis_dim_z, 2)
    vis_z_params = torch.stack(vis_z_params_f, dim=1).view(batch_size, T, svae.vis_dim_z, 2)
    vis_zs = torch.stack(vis_zs_f, dim=1).view(batch_size, T, svae.vis_dim_z)

    vis_x_recon_params = torch.stack(vis_x_hat_params_f, dim=1)
    vis_x_t_1 = torch.stack(vis_x_f, dim=1)

    vis_logpx = svae.vis_vae.x_dist.log_density(vis_x_t_1, params=vis_x_recon_params).view(batch_size, -1).sum(1)
    vis_logpz = svae.vis_vae.prior_dist.log_density(vis_zs, params=vis_prior_params_z).view(batch_size, -1).sum(1)
    vis_logqz_condx = svae.vis_vae.q_dist.log_density(vis_zs, params=vis_z_params).view(batch_size, -1).sum(1)

    vis_recon = (torch.mean(vis_logpx) + hyperparam['lamb']*torch.mean(vis_logpz - vis_logqz_condx))

    # ############################################### Tactile #######################################
    tac_prior_params_z = torch.stack(tac_prior_params_z_f, dim=1).view(batch_size, T, svae.tac_dim_z, 2)
    tac_z_params = torch.stack(tac_z_params_f, dim=1).view(batch_size, T, svae.tac_dim_z, 2)
    tac_zs = torch.stack(tac_zs_f, dim=1).view(batch_size, T, svae.tac_dim_z)

    tac_x_recon_params = torch.stack(tac_x_hat_params_f, dim=1)
    tac_x_t_1 = torch.stack(tac_x_f, dim=1)

    tac_logpx = svae.tac_vae.x_dist.log_density(tac_x_t_1, params=tac_x_recon_params).view(batch_size, -1).sum(1)
    tac_logpz = svae.tac_vae.prior_dist.log_density(tac_zs, params=tac_prior_params_z).view(batch_size, -1).sum(1)
    tac_logqz_condx = svae.tac_vae.q_dist.log_density(tac_zs, params=tac_z_params).view(batch_size, -1).sum(1)
...
def main():
    ...
    parser.add_argument('--stdy', default=0.05, type=float, help='standard deviation of tactile y space')
    ...
    parser.add_argument('--save', default=os.path.join('results', 'baseline'), help='Path to save the models')
    ...
    if args.validate:
        val_dir = os.path.join('dataset', 'cm_dataset', 'val_set')
    ...
    # Setup the optimizer
    optimizer = optim.Adam(svae.parameters(), lr=args.learning_rate)
    ...
    while iteration < num_iterations:
        for i, values in enumerate(train_loader):
            ...
            obj, elbo = compute_elbo(svae, vis_obs, tac_obs, actions, labels, hyperparam)
            ...
            if iteration % args.log_freq == 0:
                ...
                utils.save_checkpoint({
                    'state_dict': svae.state_dict(),
                    'args': args}, checkpoint_dir, iteration)

                if args.validate:
                    svae.eval()
                    validate_baseline(val_loader, svae, out_dir, iteration, save_plot=args.saveplots, show_plot=args.showplots, vis=vis)
